...
const Item = (props) => {
    return (
        <div className="w-64 text-center bg-white rounded-2xl mb-8 shadow-xl pb-4 hover:drop-shadow-2xl transition-all ease-in-out duration-500">
            <Link to={`/san-pham/${props.id}`} onClick={window.scrollTo(0, 0)} className="hover:text-[#02c4c1] transition-all ease-in duration-200 "><img src={`${process.env.REACT_APP_URL_API}/images/products/${props.id}/${props.thumbnail}`} alt="nothing" className="rounded-tr-2xl rounded-tl-2xl" /></Link>

...
const Header = () => {
    ...
    const filterProducts = (value) => {
        const filtered = data.filter((product) => {
            return value && product.name && product.name.toLowerCase().includes(value);
        });
        ...
        <header className="w-full">
            <div className="flex items-center justify-between mx-48 h-20">
                <div className="flex items-center">
                    ...
                    <div className="h-full w-72 mr-6 py-2">
                        ...
                        <a href="/#" rel="home" className="flex items-center">
                            <img loading="lazy" src={`${process.env.REACT_APP_URL_API}/GAUBONG-THEOYEUCAUCUANGUOILA.png`} alt="Gấu bông online" className="h-16" />
                            <span className="text-[#02c4c1] uppercase text-xl font-bold">Gấu Bông Online</span>
                        </a>
                    </div>
                </div>
                <div className="relative border rounded-sm border-gray-200 hover:border-gray-400 active:border-gray-400 focus-within:border-gray-400 bg-gray-100 p-2 flex ">
                    ...
                    <input type="text" placeholder="Search" className="w-80 outline-none bg-gray-100" value={searchInput} onChange={handleChange} />
                    {
                        filteredProducts.length > 0 ? (
                            <div className="absolute bg-black h-auto w-full top-[51.6px] left-0 z-50">
                                {
                                    filteredProducts.map((item, i) => {
                                        return <div className="w-64 text-center bg-white rounded-2xl mb-8 shadow-xl pb-4 hover:drop-shadow-2xl transition-all ease-in-out duration-500">
                                            <Link to={`/san-pham/${item.id}`} onClick={window.scrollTo(0, 0)} className=""><img src={`${process.env.REACT_APP_URL_API}/products/${item.id}/${item.thumbnail}`} alt="nothing" className="rounded-2xl" /></Link>
                                            <p className="pt-2"><Link to={`/san-pham/${item.id}`} className="hover:text-[#02c4c1] transition-all ease-in duration-200 font-medium block lowercase first-letter:uppercase min-h-12">{item.name}</Link></p>
                                            <p className="font-semibold py-2 cursor-default">{Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(item.price)}</p>
